...
class IMatrixMSEObserver(Observer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not (0 <= self.maxgrow <= 0.5):
            raise ValueError(f"maxgrow must be in [0, 0.5], got {self.maxgrow}")
    ...
    def _prepare_importance(self, observed: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        # to handle all strategies (CHANNEL, GROUP, BLOCK) and g_idx
...
def _grid_search(
    ...
    grow_steps = max(0, int(maxgrow * grid))
    shrink_steps = max(1, int(maxshrink * grid))
    for i in range(-grow_steps, shrink_steps + 1):
        p = 1 - i / grid
